      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          ...
          <div className="p-6 rounded-3xl bg-gradient-to-br from-emerald-50/80 to-teal-50/40 border border-emerald-200/80 shadow-sm hover:shadow-md transition-shadow">
            ...
            />
            <EditableText
              value={siteConfig.overviewAudienceDescription || 'Personas, familias y participantes interesados en soluciones ecológicas que reducen el uso continuo de envolturas y plásticos convencionales.'}
              onSave={(val) => updateSiteConfig({ overviewAudienceDescription: val })}
              tagName="p"
              multiline
          ...
          <div className="p-6 rounded-3xl bg-gradient-to-br from-teal-50/80 to-emerald-50/40 border border-teal-200/80 shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="mt-4 pt-3 border-t border-teal-200/60 flex items-center gap-1 text-xs text-teal-800 font-semibold">
              ...
              <EditableText
                value={siteConfig.overviewAudienceScope || '100 Participantes directos en talleres'}
                onSave={(val) => updateSiteConfig({ overviewAudienceScope: val })}
                tagName="span"
              />
            </div>
          </div>
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-4 flex flex-col gap-3">
              <div className="p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm text-center">
                <EditableText
                  value={siteConfig.overviewStatPlasticTime || '100 a 500'}
                  onSave={(val) => updateSiteConfig({ overviewStatPlasticTime: val })}
                  tagName="p"
                  className="text-3xl font-extrabold text-emerald-400 block"
                />
                <EditableText
                  value={siteConfig.overviewStatPlasticLabel || 'Años que tarda en degradarse el plástico sintético tradicional'}
                  onSave={(val) => updateSiteConfig({ overviewStatPlasticLabel: val })}
                  tagName="p"
                  className="text-xs text-slate-300 mt-1 block"
                />
              </div>
              <div className="p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm text-center">
                <EditableText
                  value={siteConfig.overviewStatSolviplasTime || 'Minutos'}
                  onSave={(val) => updateSiteConfig({ overviewStatSolviplasTime: val })}
                  tagName="p"
                  className="text-3xl font-extrabold text-teal-300 block"
                />
                <EditableText
                  value={siteConfig.overviewStatSolviplasLabel || 'Tiempo de disolución limpia de Solviplas en agua tibia'}
                  onSave={(val) => updateSiteConfig({ overviewStatSolviplasLabel: val })}
                  tagName="p"
                  className="text-xs text-slate-300 mt-1 block"
                />
              </div>
