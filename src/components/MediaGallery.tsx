...
export const MediaGallery: React.FC = () => {
  const { siteConfig, updateSiteConfig, mediaItems, isAdmin, isLiveEditEnabled, addMediaItem, updateMediaItem, deleteMediaItem, toggleFeaturedMedia, showToast } = useApp();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            ...
            <EditableText
              value={siteConfig.galleryTitle || 'Galería Multimedia de Solviplas'}
              onSave={(val) => updateSiteConfig({ galleryTitle: val })}
              tagName="h2"
              className="text-3xl sm:text-4xl font-extrabold tracking-tight text-white block"
            />
            <EditableText
              value={siteConfig.gallerySubtitle || 'Fotos y videos de los talleres prácticos, las pruebas de solubilidad y los prototipos finales biodegradables.'}
              onSave={(val) => updateSiteConfig({ gallerySubtitle: val })}
              tagName="p"
              multiline
              className="mt-2 text-sm sm:text-base text-slate-300 max-w-xl block"
            />
          </div>
