...
export const EditableMedia: React.FC<EditableMediaProps> = ({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showToast(`¡${result.type === 'video' ? 'Video' : 'Imagen'} subido a la nube! Pulsa "Guardar Cambios" para aplicar.`, 'success');
    } catch (err) {
      console.error('Upload error:', err);
      showToast('Error al subir archivo a Google Cloud. Intenta de nuevo.', 'error');
    } finally {
      setIsUploading(false);
    }
  };
  ...
  const handleApplyChanges = () => {
    ...
    setIsOpen(false);
    showToast('¡Multimedia actualizada con éxito en la nube!', 'success');
  };
            ...
            {activeTab === 'upload' && (
              <div className="space-y-3">
                ...
                <button
                  ...
                  className="w-full p-6 rounded-2xl border-2 border-dashed border-emerald-400 bg-emerald-50/40 hover:bg-emerald-50 text-center cursor-pointer transition-colors flex flex-col items-center justify-center gap-2 disabled:opacity-50"
                >
                  <div className="w-11 h-11 rounded-2xl bg-emerald-600 text-white flex items-center justify-center shadow-md">
                    <Upload className="w-5 h-5" />
                  </div>
                  <span className="text-xs font-bold text-emerald-950">
                    Seleccionar {selectedType === 'video' ? 'Video' : 'Imagen'} descargado en tu dispositivo
                  </span>
                  <span className="text-[11px] text-slate-500">
                    Se sube a Google Cloud Firestore y será visible por todos los usuarios
                  </span>
                </button>
