  ...
  return (
    <section id="products" className="py-24 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-white relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Header */}
        <div className="text-center max-w-3xl mx-auto mb-12 scroll-fade-item">
        ...
        {/* Categories Bar & Admin Add Product Button with Liquid Styling */}
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-10 pb-4 border-b border-emerald-900/40 relative">
          
          {/* Horizontal scrollable liquid category pills */}
          <div className="flex items-center gap-2 overflow-x-auto w-full sm:w-auto pb-2 sm:pb-0 no-scrollbar py-1">
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`relative text-xs font-bold capitalize whitespace-nowrap transition-all duration-200 cursor-pointer overflow-hidden ${
                    isSelected
                      ? 'px-4 py-1.5 rounded-full bg-gradient-to-r from-emerald-300 via-teal-200 to-cyan-300 text-slate-950 font-black shadow-[0_2px_14px_rgba(52,211,153,0.5),inset_0_1px_1px_rgba(255,255,255,0.9),inset_0_-2px_4px_rgba(13,148,136,0.3)] ring-1 ring-emerald-200/90 scale-105'
                      : 'px-3.5 py-1.5 rounded-full bg-emerald-950/60 text-emerald-200/80 hover:text-white hover:bg-emerald-900/60 border border-emerald-800/40'
                  }`}
                >
                  {isSelected && (
                    <span className="absolute top-0.5 inset-x-2 h-[40%] rounded-full bg-gradient-to-b from-white/70 to-transparent pointer-events-none" />
                  )}
          ...
          {canEdit && (
            <button
              type="button"
              onClick={() => addProduct()}
              className="flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-slate-950 font-black text-xs shadow-[0_0_16px_rgba(52,211,153,0.4)] transition-all hover:scale-105 shrink-0 cursor-pointer"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {filteredProducts.map((prod) => (
            <div
              key={prod.id}
              className="liquid-glass-card group rounded-3xl overflow-hidden transition-all duration-300 hover:-translate-y-1.5 flex flex-col justify-between scroll-fade-item relative"
            >
