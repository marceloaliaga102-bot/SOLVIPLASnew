    ...
    <section id="problem" className="py-16 sm:py-24 bg-gradient-to-b from-white to-emerald-50/50 border-b border-emerald-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-14">
          ...
          <EditableText
            value={siteConfig.treeProblemTitle || 'La Problemática de los Plásticos de Un Solo Uso'}
            onSave={(val) => updateSiteConfig({ treeProblemTitle: val })}
            tagName="h2"
        ...
        {activeTree === 'problem' ? (
          <div className="p-6 sm:p-10 rounded-3xl bg-white border-2 border-rose-200/80 shadow-xl space-y-8 animate-fadeIn">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {(siteConfig.treeProblemConsequences || [
                  ...
                  <div key={idx} className="p-4 rounded-2xl bg-rose-50 border border-rose-200 text-center shadow-sm">
                    <EditableText
                      value={item.title}
                      onSave={(val) => {
                        const current = [...(siteConfig.treeProblemConsequences || [])];
                        current[idx] = { ...current[idx], title: val };
                        updateSiteConfig({ treeProblemConsequences: current });
                      }}
                      tagName="h4"
                      className="text-xs sm:text-sm font-bold text-rose-950 mb-1 block"
                    />
                    <EditableText
                      value={item.desc}
                      onSave={(val) => {
                        const current = [...(siteConfig.treeProblemConsequences || [])];
                        current[idx] = { ...current[idx], desc: val };
                        updateSiteConfig({ treeProblemConsequences: current });
                      }}
                      tagName="p"
                      multiline
                      className="text-[11px] text-rose-800 leading-relaxed block"
                    />
                  </div>
                ))}
              </div>
            ...
            <div className="relative py-2 flex flex-col items-center">
              ...
              <div className="w-full max-w-xl p-5 rounded-2xl bg-gradient-to-r from-rose-600 to-red-600 text-white text-center shadow-lg ring-4 ring-rose-200">
                ...
                <EditableText
                  value={siteConfig.treeProblemCentral || "Contaminación ambiental acelerada por el consumo masivo de polímeros plásticos sintéticos no biodegradables"}
                  onSave={(val) => updateSiteConfig({ treeProblemCentral: val })}
                  tagName="h3"
                  multiline
                ...
                {(siteConfig.treeProblemCauses || [
                  ...
                  <div key={idx} className="p-4 rounded-2xl bg-amber-50/80 border border-amber-200 text-center shadow-sm">
                    <EditableText
                      value={item.title}
                      onSave={(val) => {
                        const current = [...(siteConfig.treeProblemCauses || [])];
                        current[idx] = { ...current[idx], title: val };
                        updateSiteConfig({ treeProblemCauses: current });
                      }}
                      tagName="h4"
                      className="text-xs sm:text-sm font-bold text-amber-950 mb-1 block"
                    />
                    <EditableText
                      value={item.desc}
                      onSave={(val) => {
                      ...
                      tagName="p"
                      multiline
                      className="text-[11px] text-amber-800 leading-relaxed block"
                    />
                  </div>
                ))}
              </div>
          ...
          <div className="p-6 sm:p-10 rounded-3xl bg-white border-2 border-emerald-200/80 shadow-xl space-y-8 animate-fadeIn">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {(siteConfig.treeObjectiveEffects || [
                  ...
                  <div key={idx} className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-center shadow-sm">
                    <EditableText
                      value={item.title}
                      onSave={(val) => {
                        const current = [...(siteConfig.treeObjectiveEffects || [])];
                        current[idx] = { ...current[idx], title: val };
                        updateSiteConfig({ treeObjectiveEffects: current });
                      }}
                      tagName="h4"
                      className="text-xs sm:text-sm font-bold text-emerald-950 mb-1 block"
                    />
                    <EditableText
                      value={item.desc}
                      onSave={(val) => {
                      ...
                      tagName="p"
                      multiline
                      className="text-[11px] text-emerald-800 leading-relaxed block"
                    />
                  </div>
                ))}
              </div>
            ...
            <div className="relative py-2 flex flex-col items-center">
              ...
              <div className="w-full max-w-xl p-5 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 text-white text-center shadow-lg ring-4 ring-emerald-200">
                ...
                <EditableText
                  value={siteConfig.treeObjectiveCentral || "Implementar bioplásticos hidrosolubles y biodegradables accesibles para reemplazar materiales sintéticos"}
                  onSave={(val) => updateSiteConfig({ treeObjectiveCentral: val })}
                  tagName="h3"
                  multiline
                  className="text-base sm:text-lg font-extrabold leading-snug block"
                />
                ...
                {(siteConfig.treeObjectiveMeans || [
                  ...
                  <div key={idx} className="p-4 rounded-2xl bg-teal-50/80 border border-teal-200 text-center shadow-sm">
                    <EditableText
                      value={item.title}
                      onSave={(val) => {
                        const current = [...(siteConfig.treeObjectiveMeans || [])];
                        current[idx] = { ...current[idx], title: val };
                        updateSiteConfig({ treeObjectiveMeans: current });
                      }}
                      tagName="h4"
                      className="text-xs sm:text-sm font-bold text-teal-950 mb-1 block"
                    />
                    <EditableText
                      value={item.desc}
                      onSave={(val) => {
                      ...
                      tagName="p"
                      multiline
                      className="text-[11px] text-teal-800 leading-relaxed block"
                    />
                  </div>
                ))}
              </div>
